      ...
      $.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) {
        ...
        // if the character is not an underscore (empty space)
          ? (function (naming) {
            ...
            var id = overrideId || naming.getId(character, naming.rows[row], naming.columns[column])
            seats[id] = new seat({
              id: id,
              label: overrideLabel || naming.getLabel(character, naming.rows[row], naming.columns[column]),
              row: row,
              column: column,
              character: character
            })
    ...
    fn.data('seatCharts', {
      ...
      each: function (callback) {
        ...
        for (var seatId in fn.seats) {
          if (callback.call(fn.seats[seatId], seatId) === false) {
